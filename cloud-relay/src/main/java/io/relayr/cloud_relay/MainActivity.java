...
public class MainActivity extends Activity {
    ...
    private static final Device mNullableDevice = new Device("", "", "", "");
    ...
    private void fetchDevices() {
        (!RelayrSdk.isUserLoggedIn() ? RelayrSdk.logIn(this) : RelayrSdk.getUser())
                ...
                .subscribe(new Subscriber<List<Device>>() {
                    ...
                    @Override public void onNext(List<Device> devices) {
                        for (Device device : devices) {
                            if (device.getId().equals(BUTTON)) {
                                mButtonDevice = device;
                                subscribeToButtonClickPushes();
                            } else if (device.getId().equals(SIREN_LIGHT)) {
                                mSirenLightDevice = device;
                            } else if (device.getId().equals(NOISE_LEVEL)) {
